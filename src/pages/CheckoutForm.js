...
const CheckoutForm = ({ cartItems, totalPrice }) => {
  ...
  useEffect(() => {
    fetch('https://capstone-a5ic.onrender.com/create-payment-intent', {
      method: 'POST',
    ...
    if (error) {
      ...
      try {
        for (const item of cartItems) {
          const response = await fetch('https://capstone-a5ic.onrender.com/api/events/reduce-tickets', {
            method: 'PUT',
            headers: {
        ...
        await fetch('https://capstone-a5ic.onrender.com/api/bookings', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
